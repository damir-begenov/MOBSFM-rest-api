...
router.post('/getReceivedMessages', (req, res) => {
    const {organization_id} = req.body;
    db.task(async t => {
        const messReceived = await t.many('SELECT * from correspondence c inner join correspondence_receiver cr on c.id = cr.correspondence_id where cr.organization_id = $1', [organization_id]);
        res.json({
