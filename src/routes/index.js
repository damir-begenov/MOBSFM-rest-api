...
router.get('/assessment', (req, res) => {
    // const {organization_id} = req.body;
    db.task(async t => {
        const assessment = await t.manyOrNone(`SELECT * FROM assessments_assessment where organization_id = $1 INNER JOIN assessnments_assessmentitem ON assessments_assessment.id = assessnments_assessmentitem.assessment_id `);
...
router.post('/checkCountOrg', (req, res) => {
    ...
    db.task(async t => {
        ...
        if (user) {
            ...
            const organization = await t.many('SELECT * FROM accounts_organization a inner join directories_codetype b on  a.subject_code_id = b.id WHERE a.id in (SELECT organization_id FROM accounts_employee WHERE client_user_id = $1)', [user['id']]);
            if (organization) {
